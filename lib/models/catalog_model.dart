...
class Sport {
  ...
  /// The primary (Arabic) rendering. Also what `name` comes back as when
  /// the API is asked in Arabic, which is what this panel always does.
  String? name;

  /// The optional English rendering, shown to members who run the app in
  /// English. Blank means "fall back to the Arabic name" — the backend
  /// does that, not the client.
  String? nameEn;

  String? description;
  String? descriptionEn;

  /// A key from the shared icon catalogue, not a font codepoint.
  /// See [SportIcons] for the glyph it maps to.
  String? icon;

  /// The stored value — a disk path or an external link. Write this back
  /// verbatim; use [imageUrl] to display.
  String? image;

  /// Ready-to-render absolute URL, built server-side.
  String? imageUrl;
  ...
  factory Sport.fromJson(Map<String, dynamic> json) => Sport(
    ...
    // resolved for the request's language. The panel edits the raw
    ...
    description:
        asString(json['description_ar']) ?? asString(json['description']),
    descriptionEn: asString(json['description_en']),
  ...
  Map<String, dynamic> toJson() => {
    ...
    // Sent even when empty so clearing the English field actually clears
    // it server-side rather than silently keeping the old translation.
    'name_en': (nameEn ?? '').isEmpty ? null : nameEn,
    if (description != null) 'description': description,
    'description_en': (descriptionEn ?? '').isEmpty ? null : descriptionEn,
    'icon': icon,
  };

  /// True when the sport has artwork to draw; otherwise the icon is used.
...
class Membership {
  ...
  /// Stored value (disk path or external link); [imageUrl] is what renders.
  String? image;
  String? imageUrl;

  /// The sport's icon key, so a membership row can show the sport's glyph
  ...
  /// True when the membership has artwork for the card backdrop.
  bool get hasImage => (imageUrl ?? '').isNotEmpty;
...
class MembershipSchedule {
  ...
  factory MembershipSchedule.fromJson(Map<String, dynamic> json) =>
      MembershipSchedule(
        ...
        specificDate: asDate(json['specific_date']),
