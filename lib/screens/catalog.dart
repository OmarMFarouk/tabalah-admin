...
class _CatalogView extends StatelessWidget {
      ...
      case CatalogTab.memberships:
        return AppTable<Membership>(
          ...
          rowBuilder: (rc, m, i) => AppRow(
            index: i,
            cells: [
              // The membership's own artwork if it has any, otherwise the
              // sport's glyph - the same precedence the member app uses,
              // so a row here looks like the card there.
              mediaCell(
                rc,
                m.name ?? '—',
                flex: 3,
                sub: m.nameEn ?? m.description,
                leading: SportBadge(
                  imageUrl: m.imageUrl,
                  icon: m.sportIcon,
                ),
          ...
          rowBuilder: (rc, s, i) => AppRow(
            ...
            cells: [
              mediaCell(
                ...
                s.name ?? '—',
                flex: 3,
                // Surfacing the English name in the row is what makes a
                // missing translation visible at a glance rather than a
                // surprise reported from the member app.
                sub: s.nameEn ?? '— لا توجد ترجمة إنجليزية',
                leading: SportBadge(imageUrl: s.imageUrl, icon: s.icon),
              ),
  ...
  void _openForm(
    ...
    if (membership != null) {
      ...
      c.nameEnCont.text = membership.nameEn ?? '';
      c.descCont.text = membership.description ?? '';
      c.descEnCont.text = membership.descriptionEn ?? '';
      ...
      // See the sport branch: only an external link is editable as text.
      c.imageCont.text = (membership.image ?? '').startsWith('http')
          ? membership.image!
          : '';
    }
    if (sport != null) {
      c.nameCont.text = sport.name ?? '';
      c.nameEnCont.text = sport.nameEn ?? '';
      c.descCont.text = sport.description ?? '';
      c.descEnCont.text = sport.descriptionEn ?? '';
      c.formIcon = sport.icon;
      // Only an external link goes back into the URL field. A stored disk
      // path is not something an admin can retype, and putting it here
      // would re-send it as `image_url` on save and fail validation.
      c.imageCont.text = (sport.image ?? '').startsWith('http')
          ? sport.image!
          : '';
    }
    if (schedule != null) {
...
class _CatalogFormState extends State<_CatalogForm> {
  ...
  Widget _sportForm(CatalogCubit c) {
    ...
    return AppDialog<CatalogCubit>(
      ...
      child: Column(
        ...
        children: [
          // Arabic and English side by side rather than in separate tabs:
          // the whole point is that an admin filling one notices the other
          // is empty, because a blank English name is what leaves members
          // reading Arabic in an English app.
          dialogRow([
            AppField(
              controller: c.nameCont,
              label: 'اسم الرياضة (عربي) *',
              icon: Icons.sports_soccer_rounded,
          ...
          IconPickerField(
            value: c.formIcon,
            label: 'أيقونة الرياضة',
            onChanged: (key) => setState(() => c.formIcon = key),
          ),
          gap,
          // Image wins over icon in both clients when both are set, so the
          // icon is still worth picking as the fallback.
          ImageField(
            urlController: c.imageCont,
            label: 'صورة الرياضة',
            hint: 'إن رُفعت صورة تُعرض بدلاً من الأيقونة',
            pickedPath: c.pendingImagePath,
            currentUrl: widget.sport?.imageUrl,
            markedForRemoval: c.clearImage,
            onPickFile: () async {
              await c.pickImageFile();
              if (mounted) setState(() {});
            },
            onDropPicked: () => setState(c.dropPickedImage),
            onRemove: () => setState(c.markImageForRemoval),
          ),
        ],
      ),
    );
  ...
  // ── Membership ──────────────────────────────
  /// The coaches attached to [sportId]; empty while no sport is chosen.
  List<TrainerProfile> _sportTrainers(int? sportId) => sportId == null
      ? const []
      : AppGlobals.trainers.where((t) => t.sportId == sportId).toList();
    ...
    return AppDialog<CatalogCubit>(
      ...
      child: Column(
        ...
        children: [
          ...
          dialogRow([
            AppDropdown<Sport>(
              ...
              onChanged: (s) => setState(() {
                c.formSportId = s?.id;
                // The coach list below is scoped to the sport, so a coach
                // picked under the old one would silently stay attached to a
                // sport they don't teach. Drop them unless they carry over.
                if (!_sportTrainers(c.formSportId)
                    .any((t) => t.id == c.formTrainerId)) {
                  c.formTrainerId = null;
                }
              }),
            ),
            // Deliberately empty until a sport is chosen: a coach only ever
            // belongs to one sport, so offering the whole roster first just
            // invites a mismatch the server would reject.
            AppDropdown<TrainerProfile>(
              value: pickWhere(
                _sportTrainers(c.formSportId),
                (t) => t.id == c.formTrainerId,
              ),
              items: _sportTrainers(c.formSportId),
              labelOf: (t) => t.displayName,
              label: 'المدرب *',
              icon: Icons.sports_rounded,
              emptyLabel: c.formSportId == null
                  ? 'اختر الرياضة أولاً'
                  : 'لا يوجد مدربون لهذه الرياضة',
          ...
          gap,
          // Drawn behind the membership card at low opacity in the member
