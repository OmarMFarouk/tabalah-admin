...
class PeopleApi {
  ...
  // Add a member: either a brand new account (name/email/password) or by
  // promoting an existing one (user_id). Same dual shape as createEmployee.
  Future<ApiResponse> createPlayer(Map<String, dynamic> data) =>
      ApiClient.post(AppEndPoints.players, data: data);

  // ── Parent portal — بوابة ولي الأمر ─────────
  //  Issuing a new code invalidates the old one and drops any parent
  //  session opened with it. Disabling keeps the code but refuses it at
  //  the door, which is the reversible option.
  Future<ApiResponse> rotateGuardianCode(dynamic playerId) =>
      ApiClient.post(AppEndPoints.playerGuardianCode(playerId));

  Future<ApiResponse> setGuardianAccess(dynamic playerId, bool enabled) =>
      ApiClient.patch(
        AppEndPoints.playerGuardianAccess(playerId),
